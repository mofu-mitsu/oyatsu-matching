...
// 1. fallbacksを条件なしで取得するように変更（必ず複数出るように）
const fallbackRegex = /const fallbacks = getFallbackItems\(typeInfo\.id, minP, maxP, flavors, allDislikes\);/g;
code = code.replace(fallbackRegex, "const fallbacks = getFallbackItems(typeInfo.id, 0, 99999, [], []);");

// 2. モーダルstateとスマホ判定の追加
if (!code.includes('showImageModal')) {
...
// 4. handleShare (画像シェア) と handleTextShare (テキストシェア)
...
// 5. JSXにハンドル追加（すでに修正されているが、handleTextShareが呼ばれるのでOK）
// ただしモーダルのJSXが含まれていない場合は末尾に追加する
if (!code.includes('setShowImageModal(null)')) {
  const jsxRegex = /(<\/div>\s*)$/m; // JSXの最後らへん
  // </button></div> のうしろ
  const buttonsRegex = /(<\/button>\s*<\/div>\s*)$/m;
  const modalJsx = `
      {showImageModal && (
  ...
  code = code.replace(buttonsRegex, '$1' + modalJsx);
}
