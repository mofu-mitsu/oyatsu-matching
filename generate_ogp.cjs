...
const content = fs.readFileSync('src/data/snackTypes.ts', 'utf-8');
const publicDir = path.join(process.cwd(), 'public');
if (!fs.existsSync(publicDir)) {
  fs.mkdirSync(publicDir, { recursive: true });
}

const typeRegex = /id:\s*'([A-Z]{4})',[\s\S]*?title:\s*'([^']+)',[\s\S]*?description:\s*'([^']+)',[\s\S]*?personalityTraits:\s*\[([^\]]+)\],[\s\S]*?characterName:\s*'([^']+)'/g;
...
while ((match = typeRegex.exec(content)) !== null) {
  ...
  const traitsRaw = match[4];
  const traits = traitsRaw.split(',').map(s => s.replace(/['"\s]+/g, '')).filter(Boolean);
  const characterName = match[5];
...
<head>
  ...
  <title>おやつ16タイプ診断結果: ${characterName}</title>
  
  <meta property="og:title" content="おやつ16タイプ診断結果: ${characterName}" />
  <meta property="og:description" content="私の診断結果は「${title}」でした！あなたにぴったりのおやつを見つけよう！" />
  <meta property="og:image" content="https://oyatsu-matching.vercel.app/ogp/${typeCode}.png" />
  <meta property="og:url" content="https://oyatsu-matching.vercel.app/${typeCode}.html" />
...
<body class="bg-stone-50 min-h-screen text-stone-800 font-sans pb-24">
  <div class="max-w-md mx-auto bg-white min-h-screen shadow-xl relative">
    ...
    <div class="p-6">
      <div class="text-center mb-6 mt-2">
        ...
        <h1 class="text-3xl font-extrabold text-stone-800 mb-3">${characterName}</h1>
      ...
      <div class="mb-8 rounded-3xl overflow-hidden shadow-md border-[6px] border-stone-50 bg-stone-100 aspect-square flex items-center justify-center relative group">
        <img src="/ogp/${typeCode}.png" alt="${characterName}" class="w-full h-full object-cover transition-transform duration-500 group-hover:scale-105" onerror="this.src='https://placehold.co/600x600/f5f5f4/a8a29e?text=No+Image'" />
      </div>
      ...
      <div class="bg-stone-50 rounded-3xl p-6 mb-6 border border-stone-100 shadow-sm">
        ...
        <p class="text-sm leading-relaxed text-stone-600 font-medium">${description}</p>
      </div>
      ...
      <div class="bg-stone-50 rounded-3xl p-6 mb-8 border border-stone-100 shadow-sm">
        ...
        <div class="flex flex-wrap gap-2">
          ${traits.map(t => `<span class="bg-white border border-stone-200 text-stone-700 px-3 py-1.5 rounded-full text-xs font-bold shadow-sm">${t}</span>`).join('')}
        </div>
      </div>
  ...
  fs.writeFileSync(path.join(publicDir, `${typeCode}.html`), html);
  console.log(`Generated ${typeCode}.html`);
}
